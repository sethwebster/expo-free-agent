...
export function createDoctorCommand(): Command {
  ...
  command
    ...
    .action(async (workerId?: string, options?: { latest?: boolean; limit?: string }) => {
      try {
        ...
        if (options?.latest) {
          const report = await apiClient.getLatestDiagnostic(workerId);
          displayDiagnosticReport(report);
        } else {
          const limit = parseInt(options?.limit || '10');
          const data = await apiClient.getDiagnostics(workerId, limit);
