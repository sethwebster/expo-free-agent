...
export function NetworkHUD() {
  const { stats, isLive } = useNetworkContext();

  return (
    <div className="flex justify-center pointer-events-none">
      <div className="p-1 rounded-2xl bg-zinc-50/50 dark:bg-zinc-900/50 backdrop-blur-md border border-zinc-200/50 dark:border-zinc-800/50 shadow-xl inline-flex flex-col sm:flex-row gap-0 sm:gap-2 items-center overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-6 py-3 bg-white/50 dark:bg-black/50 rounded-xl w-full sm:w-auto justify-center sm:justify-start">
          <span className="relative flex h-3 w-3">
            ...
            <span className={`relative inline-flex rounded-full h-3 w-3 ${isLive ? 'bg-emerald-500' : 'bg-zinc-400 dark:bg-zinc-600'}`}></span>
          </span>
